...
#                                                     device_type=device)
    if provider == 'triton-cpu-single':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: matmul(triton_a, triton_b, triton_c, m_dim, n_dim, k_dim, k_block, num_threads=1), rep=1000, quantiles=quantiles)
    elif provider == 'triton-cpu' or provider == 'triton-cpu-single':
        ...
        assert(False and "unexpected provider")
